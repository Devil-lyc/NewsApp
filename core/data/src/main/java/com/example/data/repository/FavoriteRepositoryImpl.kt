...
import com.example.network.FavoriteApiService
...
//            Log.e(TAG, "获取收藏列表异常: ${e.message}")
     ...
     * 获取收藏详情
     */
    override suspend fun getFavoriteById(id: String): Flow<Resource<com.example.model.Favorite>> = flow {
        emit(Resource.Loading())
        try {
...
//            Log.e(TAG, "获取收藏详情异常: ${e.message}")
    ...
    override suspend fun addFavorite(
        ...
        author: String?,
        newsId: String?
    ): Flow<Resource<com.example.model.Favorite>> = flow {
        emit(Resource.Loading())
        try {
            // 记录请求参数日志
