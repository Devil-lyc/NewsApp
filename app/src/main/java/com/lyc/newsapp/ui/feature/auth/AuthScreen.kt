...
fun AuthScreen(
    ...
    LaunchedEffect(authState.isLoginSuccessful) {
        if (authState.isLoginSuccessful) {
            viewModel.dispatch(AuthIntent.ResetLoginSuccess)
        }
    }
    ...
    LaunchedEffect(authState.isRegisterSuccessful) {
        if (authState.isRegisterSuccessful) {
            viewModel.dispatch(AuthIntent.ResetRegisterSuccess)
        }
    }
        ...
        composable(AuthRoute.Login.route) {
            LoginScreen(
                ...
                onLogin = { email, password ->
                    viewModel.dispatch(AuthIntent.Login(email, password))
                },
        ...
        composable(AuthRoute.Register.route) {
            RegisterScreen(
                ...
                onRegister = { username, email, password ->
                    viewModel.dispatch(AuthIntent.Register(username, email, password))
                },
                onNavigateToLogin = {
