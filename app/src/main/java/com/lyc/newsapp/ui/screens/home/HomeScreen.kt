...
fun HomeScreen(
    ...
    val pullRefreshState = rememberPullRefreshState(refreshing = isRefreshing, onRefresh = {
        coroutineScope.launch {
            isRefreshing = true
            delay(1000)
            homeViewModel.onEvent(HomeUiEvents.onRefresh(category = selectedCategory.id))
            isRefreshing = false
        }
    })
        ...
        if (isAtBottom && newsList.isNotEmpty() && !homeUiState.isLoading) {
            ...
            lastNews?.let {
                homeViewModel.onEvent(
                    HomeUiEvents.onPageChange(
                        category = selectedCategory.id, page = it.nextPage
                    )
                )
            }
        }
                ...
                if (homeUiState.errorMessage != null) {
                    ...
                    Box(
                        ...
                        Column(
                            ...
                            Button(onClick = { homeViewModel.onEvent(HomeUiEvents.onRefresh(category = selectedCategory.id)) }) {
                                Icon(
